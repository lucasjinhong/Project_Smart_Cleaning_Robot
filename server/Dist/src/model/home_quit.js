...
const checkId = (id, data) => __awaiter(void 0, void 0, void 0, function* () {
    return new Promise((resolve, reject) => {
        user_db_1.User.find({ _id: id, homes: { $in: [data] } }, function (err, obj) {
            if (err) {
                result.status = 500;
                result.message = err;
                reject(result);
    ...
    return new Promise((resolve, reject) => {
        home_db_1.Home.findById({ _id: data }, (err, obj) => {
            ...
            if (obj.users.length) {
                ...
                home_db_1.Home.findByIdAndDelete({ _id: data }, (err) => {
                    if (err) {
                        result.status = 500;
                        result.message = err;
                        reject(result);
...
const quitHome = (id, data) => __awaiter(void 0, void 0, void 0, function* () {
    yield checkId(id, data);
    yield home_db_1.Home.findByIdAndUpdate(data, { $pull: { users: id } });
    yield checkHome(data);
    yield user_db_1.User.findByIdAndUpdate(id, { $pull: { homes: data } });
});
exports.default = quitHome;
//# sourceMappingURL=home_quit.js.map